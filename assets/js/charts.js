...
export const TrendChart = {
  ...
  updateChart() {
    ...
    if (trendData.length === 0) {
      // Show "collecting data" message only when there's no data at all
      const option = {
        backgroundColor: 'transparent',
        title: {
    ...
    // Create series for each language showing percentages
...
// Percentage Trend Chart Hook (shows percentage changes over time)
export const PercentageTrendChart = {
  mounted() {
    this.chart = echarts.init(this.el);
    this.zoomState = null; // Track zoom state
    this.updateChart();
    ...
    // Listen for data updates from LiveView
    this.handleEvent('update-percentage-trend-chart', ({trendData, languages}) => {
      this.el.dataset.trendData = JSON.stringify(trendData);
      this.el.dataset.languages = JSON.stringify(languages);
      this.updateChart();
    });
